...
```bash
.cursor/skills/TelegramLibrarySkill/scripts/aiogram_docs_scraper/.venv/bin/python \
  .cursor/skills/TelegramLibrarySkill/scripts/aiogram_docs_scraper/search.py "webhook FSM"
```
...
- Package: `SmartExchange/backend/telegram_app/bot/` (factory, routers, FSM, middleware).
...
Project skill path: `.cursor/skills/TelegramLibrarySkill/` (auto-discovered by Codex).

Also documented in repo root `AGENTS.md`. Optional: add a Codex User Rule  
