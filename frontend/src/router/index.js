...
const routes = [
  {
    // The marketing page. Django still owns the `/` URL so it can inject the
    // crawler-facing <head>, but it serves this same SPA shell — see
    // landing/views.py.
    path: '/',
    name: 'landing',
  ...
  {
    // Its own URL rather than a `#contact` anchor: the sales line is what a
    // visitor is sent, bookmarks and searches for.
    path: '/contact',
    name: 'contact',
...
const router = createRouter({
  ...
  routes,
  // The marketing pages link to each other by hash (`/#package`), so a hash has
  // to win over everything else; `top` clears the sticky landing header.
