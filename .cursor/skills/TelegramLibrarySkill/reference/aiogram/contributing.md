...
> Source: [https://docs.aiogram.dev/en/latest/contributing.html](https://docs.aiogram.dev/en/latest/contributing.html)
...
How to work with pull-request you can read in the [GitHub docs](https://docs.github.com/en/pull-requests/collaborating-with-pull-requests/proposing-changes-to-your-work-with-pull-requests/creating-a-pull-request)
...
This action will create a `.venv` directory with the Python binaries and then you will
...
To check it worked, use described command, it should show the `pip` version and location
...
As an alternative to the traditional `pip` and `venv` workflow, you can use [uv](https://github.com/astral-sh/uv) -
...
Change the `<language_code>` in example below to the target language code, after that
you can modify texts inside `docs/locale/<language_code>/LC_MESSAGES` as `*.po` files
by using any text-editor or specialized utilites for GNU Gettext,
for example via [poedit](https://poedit.net/).
...
`<code>` is Issue or Pull-request number, after release link to this issue will
...
it is my personal initiative ([@JRootJunior](https://t.me/JRootJunior)) and
...
you can do it on [OpenCollective](https://opencollective.com/aiogram).
